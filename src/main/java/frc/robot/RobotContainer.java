...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    switch (Constants.currentMode) {
      case REAL:
        ...
        vision =
            new Vision(
                drive::addVisionMeasurement, new VisionIOPhotonVision(bulldogCam1, robotToCamera1));
    ...
    NamedCommands.registerCommand(
        "shoot",
        new ParallelCommandGroup(new ShooterSequence(shoot, hopper), new DriveToPoseAuto(drive)));
  ...
  private void configureButtonBindings() {
    ...
    addieController.povDown().and(() -> addieBoolean).whileTrue(new SlapDown(intake));

    owenController.povDown().and(() -> owenBoolean).whileTrue(new SlapDown(intake));

    addieController.povUp().and(() -> addieBoolean).whileTrue(new IntakeUp(intake));

    owenController.povUp().and(() -> owenBoolean).whileTrue(new IntakeUp(intake));
    ...
    owenController
        .rightTrigger()
        .and(() -> owenBoolean)
        .whileTrue(new ShooterSequence(shoot, hopper));

    addieController
        .leftTrigger()
        .and(() -> addieBoolean)
        .whileTrue(new PassingSequence(shoot, hopper));
    ...
    if (Constants.currentMode == Constants.Mode.SIM) {
      addieController.rightBumper().whileTrue(new ShootSim(driveSimulation, shoot));
    }
